...
1. **注册信息**：每个任务需提供
   - 任务别名（唯一标识）
   - PID（可选；提供则采集 CPU/内存）
   - 日志源路径：支持具体文件或目录（可省略 `file://` 前缀）
     - 单文件：`C:\logs\download.log`
     - 多文件（分号分隔）：`C:\logs\a.log;C:\logs\b.log`
     - 目录：`C:\logs\download\` — 自动扫描目录下最新匹配的日志文件
     - 目录模式下按扩展名过滤（`.log`、`.txt`、`.out`），取 `mtime` 最新的文件
...
- **日志采集**（文件或目录模式）：
  - 每次采集周期读取日志文件最后 N 行（默认 50 行），为分析提供上下文
  - 支持具体文件路径或目录路径；目录模式下自动扫描匹配扩展名的最新文件
