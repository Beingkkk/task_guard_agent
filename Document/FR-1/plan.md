...
- [ ] 前端注册任务时填写目录路径 `C:\logs\` 成功注册，系统按扩展名自动扫描目录下最新日志文件。
...
class LogSource:
    ...
    path: str | None = None               # 绝对路径；多文件用分号分隔，目录以 \ 或 / 结尾
    extensions: tuple[str, ...] = (".log", ".txt", ".out")  # 目录模式下用于过滤文件
