...
本里程碑是项目最核心两条监控路径的前置条件：

1. **Bash 进程监控路径**（`bash://<command>`）—— 注册 bash 任务，后续由 FR-2 启动子进程并采集 stdout/stderr 增量。
2. **进程 CPU/内存监控路径**（注册时附带 `pid`）—— 注册 PID 任务，后续由 FR-2 用 `psutil` 采样。
...
- 日志源 URI 解析：`bash://<command>`、`file://<absolute_path>`（单文件 / 目录）
...
- 注册期校验：别名唯一、URI 合法、PID 数值合法、文件路径父目录存在
...
- [ ] 在激活的 venv 中执行 `taskguard watch 下载A pid=12345 log=file://C:\data\dl.log`，写入 `data/tasks_state.json`，进程退出后再次启动 Agent 能恢复该任务。
- [ ] `taskguard watch 下载C log=bash://wget -c https://example.com/x.zip` 可注册，`tasks_state.json` 中保存原始命令。
- [ ] `taskguard unwatch <别名>` 删除任务且立即落盘。
- [ ] `taskguard list` 输出全部任务，含别名、PID、log_source.type、created_at。
...
└── utils/
    ├── __init__.py            # 已存在
    └── log_source_uri.py      # NEW: bash:// 与 file:// 解析
...
| AD-2 | 日志源 URI 解析 | 自写 / `urllib.parse` | 自写最小解析器 | `bash://wget -c http://...` 中嵌套 `://` 会被 urllib 误解；自写显式更可靠 |
...
- `file://<path>`：`path` 必须是绝对路径，父目录存在；本身可以是文件 / 目录 / 暂不存在的文件（采集器后续创建）
...
| 异常类 | 触发条件 | 上层映射 |
|---|---|---|
| `TaskRegistrationError` | 别名重复 / URI 非法 / PID 非法 | `ToolResult(ok=False, error_code=...)` → CLI exit 2 |
...
taskguard watch demo-pid pid=$$ log=file://%TEMP%\demo.log
echo $?  # 0
