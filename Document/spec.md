...
| `config/config-claude.json` | Claude Provider 配置 | 开发者/运维 |
| `config/config-openai.json` | OpenAI-compatible Provider 配置 | 开发者/运维 |
...
| LLM | Claude / OpenAI-compatible | 用户已有访问方式，prompt caching 降低成本 |
...
| CLI 单命令模式 | GUI 为主要入口，命令行仅用于开发调试 | `python -m taskguard.api.server` |
