...
class TestTaskDefaultConstruction:
    def test_minimal(self) -> None:
        t = Task(alias="x", log_source=LogSource(type="file", path="C:\\test.log"))
        assert t.alias == "x"
    ...
    def test_with_pid(self) -> None:
        t = Task(alias="x", log_source=LogSource(type="file", path="C:\\test.log"), pid=12345)
        assert t.pid == 12345

    def test_chinese_alias(self) -> None:
        t = Task(alias="下载A", log_source=LogSource(type="file", path="C:\\test.log"))
        assert t.alias == "下载A"
...
class TestTaskValidation:
    def test_alias_with_slash(self) -> None:
        with pytest.raises(ValueError):
            Task(alias="a/b", log_source=LogSource(type="file", path="C:\\test.log"))
    ...
    def test_alias_with_null(self) -> None:
        with pytest.raises(ValueError):
            Task(alias="a\x00b", log_source=LogSource(type="file", path="C:\\test.log"))

    def test_pid_zero(self) -> None:
        with pytest.raises(ValueError):
            Task(alias="x", log_source=LogSource(type="file", path="C:\\test.log"), pid=0)

    def test_pid_negative(self) -> None:
        with pytest.raises(ValueError):
            Task(alias="x", log_source=LogSource(type="file", path="C:\\test.log"), pid=-1)
