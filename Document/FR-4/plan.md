...
- **基本指令集**：`/watch`, `/watch --revise`, `/unwatch`, `/list`, `/status`, `/update`, `/cleanup`, `/help`, `exit`/`quit`/`q`（仅文件日志源）
- **`/status` 综合状态**：`QueryStatusTool` 同时查 `TaskStore`（注册信息）+ SQLite `metrics`（最新进程指标）+ `progress`（最新进度提取）+ `logs`（最近日志），统一渲染
- **新 Tool**：`HelpTool`
...
- [ ] `/list` 返回已注册任务列表；`/watch 测试 --log file://C:\data\test.log` 成功注册任务。
- [ ] `/status <别名>` 返回综合状态：注册信息（来自 `TaskStore`） + 最新进程指标 + 最新进度记录 + 最近日志（来自 SQLite）。
...
├── tools/
│   ├── __init__.py            # MOD: 注册 HelpTool / CollectAllTool / WatchTaskTool revise；QueryStatusTool 接收 metrics_store
│   ├── query.py               # MOD: QueryStatusTool 整合 metrics/progress/logs 查询
...
| AD-8 | `/status` 数据来源 | 仅 `TaskStore` / 仅 SQLite / 两者合并 | `TaskStore` + SQLite 合并 | `/status` 同时返回注册信息（`TaskStore`）与运行时数据（最新 metrics / progress / logs，均来自 SQLite）；`/progress` 在此次合并中被取消，其功能并入 `/status` |
...
Boot
  ...
  ├── 注册所有 Tools（含 HelpTool；QueryStatusTool 接收 metrics_store）
  ├── 打印 Banner + 状态摘要
  └── 后台启动 harness.run()
        │
        ▼
...
**`/status`**：固定宽度 key-value 对齐（`_format_kv_block`，key 左对齐，冒号分隔，2 空格缩进），按段位渲染：`Basic` / `Log Source` / `Metrics (latest)` / `Progress (latest)` / `Recent Logs` / `Config` / `Runtime State`。
...
```
输入: "/watch 下载A --log file://C:\\data\\dl.log --pid 12345"
...
3. 剩余部分按 --key value 解析为 params：
   {"alias": "下载A", "log": "file://C:\\data\\dl.log", "pid": "12345"}
...
### 11.1 `QueryStatusTool`（综合状态查询）

| 维度 | 设计 |
|---|---|
| 功能 | 返回某任务的综合状态：注册信息 + 最新进程指标 + 最新进度解析 + 最近日志 |
| 参数 | `alias: str`（必填） |
| 返回 | `ToolResult(ok=True, data={...task_metadata..., "latest_metrics": {...}, "latest_progress": {...}, "recent_logs": {"lines": [...], "entry_count": N}})` |
| 错误 | `alias_not_found`（任务不存在）；无运行时数据时对应字段为缺省 |
| 实现 | 同时调用 `task_store.get(alias)` + `metrics_store.query_metrics/query_progress/query_logs(alias, since=最近24h)`，合并为单一 dict |
| 备注 | 原 `QueryProgressTool` 已并入此 Tool；`/progress` 命令同步取消 |
...
> **约束**: 仅支持文件日志源（裸路径或 `file://` 前缀）。`--pid` 与 `--log` 至少提供一个。
...
| 测试层 | 覆盖目标 | 关键用例 |
|---|---|---|
| CommandParser | `/` 命令解析 | `/watch 下载A --log file://C:\\data\\dl.log` → `watch_task` + 正确 params；` /list` → `list_tasks`；未知命令 → ParseError；无 value 的 flag |
...
| QueryStatusTool（综合） | 注册信息 + 运行时数据 | mock TaskStore + MetricsStore：`query_metrics/query_progress/query_logs` 各返回一条 → 合并到 `latest_metrics/latest_progress/recent_logs`；alias 不存在 → `alias_not_found`；metrics_store=None 时只返回注册信息 |
...
| 阶段 | 状态 | 完成标准 |
|---|---|---|
| Phase 0 — 文档定稿 | ✅ | plan.md / tasks.md 已更新（含 --revise、输出格式、CST 时区） |
| Phase 1 — Parser & Intent 测试 | ✅ | T410~T413 全部完成且测试绿 |
| Phase 2 — Tool & Shell 核心 | ✅ | T420~T424 全部完成；新增 CollectAllTool、WatchTaskTool --revise、pid_status、固定宽度输出、CST 时区 |
...
async def test_command_parser():
    ...
    # 1. /watch 解析
    cmd = parser.parse("/watch 下载A --log file://C:\\data\\dl.log --pid 12345")
    assert cmd.tool_name == "watch_task"
    assert cmd.params["alias"] == "下载A"
    assert cmd.params["log"] == "file://C:\\data\\dl.log"
...
async def test_tools():
    ...
    metrics = Mock()
    metrics.query_metrics = AsyncMock(return_value=[{
        "alias": "test", "cpu_percent": 12.3, "memory_working_set": 1024**2,
        "status": "running", "timestamp": "2026-05-10T10:00:00Z",
    }])
    metrics.query_progress = AsyncMock(return_value=[{
        "alias": "test", "percentage": 68.0, "speed": "12.5 MB/s",
        "status": "normal", "timestamp": "2026-05-10T10:00:00Z",
    }])
