...
**进程监控路径**（注册时附带 `pid` + `file://<log>`）—— 注册 PID 和日志文件，后续由 FR-2 用 `psutil` 采样进程指标，用 `FileCollector` 增量读取日志。
...
- 日志源 URI 解析：`file://<absolute_path>`（单文件或多文件，分号分隔）
...
- [ ] `taskguard watch 合并任务 --log "file://C:\logs\a.log;C:\logs\b.log" --pid 42816` 可注册，多文件路径被正确解析。
- [ ] `taskguard watch 目录任务 --log file://C:\logs\` 返回错误（目录不被支持）。
